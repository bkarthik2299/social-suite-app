...
export function hasCampaignOutput(pack: CampaignPack) {
...
function normalizeStrategy(source: JsonRecord): CampaignPack['strategy'] {
...
function normalizeSocialPosts(input: unknown): CampaignPack['socialPosts'] {
...
function normalizeGoogleAds(input: unknown): CampaignPack['googleAds'] {
  return recordArray(input).map((item, index) => {
...
function normalizeSocialAds(input: unknown): CampaignPack['socialAds'] {
  return recordArray(input).map((item, index) => {
...
function normalizeBlogOutlines(input: unknown): CampaignPack['blogOutlines'] {
  return recordArray(input).map((item, index) => {
...
function normalizeCalendar(input: unknown): CampaignPack['calendar'] {
...
function normalizeCalendarType(input: unknown, title: string): CampaignPack['calendar'][number]['type'] {
  const value = stringValue(input).toLowerCase();
  if ((calendarTypes as readonly string[]).includes(value)) return value as CampaignPack['calendar'][number]['type'];
